...
const EVENT_ID = 'b0ba3111-7980-468b-80be-eeee8e0b07c1'

const EXTRA_RECIPIENTS = [
  { email: 'cagberkc@gmail.com', first_name: 'Çağberk' },
]
...
if (regErr) {
  console.error('Fetch error:', regErr)
  process.exit(1)
}

console.log(`Found ${registrations.length} approved team registrations.`)

const memberIds = Array.from(
  new Set(
    registrations.flatMap(r => [r.captain_id, r.partner_id]).filter(Boolean)
  )
)
...
for (const reg of registrations) {
  const members = [reg.captain_id, reg.partner_id]
    .filter(Boolean)
  ...
  for (const m of members) {
    ...
    if (emailError) {
      console.error(`Failed: ${m.email}`, emailError)
    } else {
      console.log(`Sent to: ${m.email}`)
      sentEmails.add(m.email.toLowerCase())
      sentCount++
    }
  }
}
...
for (const extra of EXTRA_RECIPIENTS) {
  ...
  if (sentEmails.has(extra.email.toLowerCase())) continue

  const { error: emailError } = await resend.emails.send({
    from: 'SmashTorino <info@smashtorino.com>',
    to: extra.email,
    subject: "You're registered! Chase The Red × SmashTorino Padel Cup 🎾",
    html: buildHtml(extra.first_name),
    attachments: [
      {
        filename: 'collab-logo.png',
        content: logoBase64,
  ...
  if (emailError) {
    console.error(`Failed: ${extra.email}`, emailError)
  } else {
    console.log(`Sent to: ${extra.email}`)
    sentEmails.add(extra.email.toLowerCase())
    sentCount++
  }
}
