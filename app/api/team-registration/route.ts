...
function buildInviteEmail(args: {
  ...
  const eventDate = event.date ? new Date(event.date + 'T00:00:00').toLocaleDateString('tr-TR', {
    ...
    <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;background:#0f2318;color:#ffffff;border-radius:16px;overflow:hidden;">
      ...
      <div style="padding:32px 24px;">
        ...
        <div style="background:#1a3d2e;border:1px solid #2d5a40;border-radius:12px;padding:20px;margin:0 0 24px;">
          ...
          ${eventDate ? `<p style="margin:0 0 4px;font-size:14px;color:#e6f0ea;">📅 ${eventDate}</p>` : ''}
        ...
        <div style="text-align:center;margin:0 0 24px;">
          <a href="${acceptLink}"
             style="display:inline-block;background:#ff6b35;color:#ffffff;text-decoration:none;font-size:14px;font-weight:bold;padding:14px 28px;border-radius:8px;margin:0 6px 10px;">
            ✅ Daveti Kabul Et
          </a>
          <a href="${rejectLink}"
             style="display:inline-block;background:transparent;color:#ffffff;text-decoration:none;font-size:14px;font-weight:bold;padding:14px 28px;border-radius:8px;border:1px solid #2d5a40;margin:0 6px 10px;">
            ❌ Reddet
          </a>
        </div>

        <p style="font-size:12px;color:#9bb5a5;margin:0;text-align:center;line-height:1.5;">
          Butonlar çalışmıyorsa bu linki tarayıcına kopyala:<br/>
          <span style="color:#ff6b35;word-break:break-all;">${acceptLink}</span>
        </p>
      </div>
...
export async function POST(req: NextRequest) {
  ...
  const partner_code_raw = body.partner_code?.trim().toUpperCase()

  if (!event_id || !team_name || !partner_code_raw) {
  ...
  const partner_code = partner_code_raw.startsWith('SMASH-')
    ? partner_code_raw
    : `SMASH-${partner_code_raw}`
  ...
  const captain = user as AuthUser
  ...
  if (partnerProfile.id === captain.id) {
  ...
  const { data: existingForCaptain } = await supabaseAdmin
    ...
    .or(`captain_id.eq.${captain.id},partner_id.eq.${captain.id}`)
    .neq('status', 'rejected')
    .maybeSingle()
  ...
  const captainName = [captainProfile?.first_name, captainProfile?.last_name]
    .filter(Boolean).join(' ').trim() || captain.email || 'Bir oyuncu'
  ...
  const { data: partnerAuth } = await supabaseAdmin.auth.admin.getUserById(partnerProfile.id)
